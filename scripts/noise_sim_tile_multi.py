...
# get ivar weighting
with bench.show('Getting ivars, masks'):
    ivars = []
    ...
    covsqrt_2D, extra_header, extra_hdu = tnd.read_tiled_ndmap(fn, extra_header=['FLAT_TRIU_AXIS'], extra_hdu=['COV1D'])
...
for i, m in enumerate(maps):
    for j, split in enumerate(splits):
        ...
        with bench.show(f'Getting sim for split {split}, sim number {map_id}'):
            sim_2D = tn.get_tiled_noise_sim_multi(covsqrt_2D, ivar=ivars, flat_triu_axis=flat_triu_axis_2D,
                                                cov_1D=cov_1D, split=split, seedgen_args=seedgen_args)

        t2 = time.time(); print(f'Draw sim time: {np.round(t2-t1, 3)}')
        
        enmap.write_map(fname, sim_2D)
